...
function naxly_the_pagination2($args = array(), $echo = 1)
{
...
add_filter('naxly_redux_custom_fonts_load', 'naxly_redux_custom_fonts_load');
function naxly_redux_custom_fonts_load( $custom_font ) {
	$custom_style = '';
	...
	if ( $filename = naxly_set( $pathinfo, 'filename' ) ) {
		...
		foreach ( $extensions as $extension ) {
			$file_path = esc_url(home_url('/')) . '/wp-content/themes/naxly/assets/css/custom-fonts/' . $filename . '.' . $extension;
...
/**
 * [naxly_social_share_output description]
 *
 ...
 */
function naxly_social_share_output( $icon, $color = false ) {
...
function naxly_get_social_icons()
{
	...
	if( naxly_set( $options, 'social_media' ) && is_array( naxly_set( $options, 'social_media' ) ) )
	{
		$total = count( naxly_set( $options, 'social_media' ) ) - 2;
		foreach( naxly_set( $options, 'social_media' ) as $social_icon ){
			if( isset( $social_icon['tocopy' ] ) ) continue;
			$title = naxly_set( $social_icon, 'title');
			$class = naxly_set( $social_icon, 'class');
			$link = naxly_set( $social_icon, 'social_link');
			$icon = naxly_set( $social_icon, 'social_icon');
...
add_action('naxly_social_share_output', 'naxly_social_share_output');
...
<ul class="social-list clearfix">
    ...
    <li class="facebook"><a href="http://www.facebook.com/sharer.php?u=<?php echo esc_url(get_permalink($PostID)); ?>" target="_blank"><i class="fab fa-facebook-f"></i>Facebook</a></li>
    <?php endif;?>

    <?php if($options->get( 'twitter_sharing' )):?>
    <li class="twitter"><a href="https://twitter.com/share?url=<?php echo esc_url(get_permalink($PostID)); ?>&text=<?php echo esc_attr($post_slug=$PostName); ?>" target="_blank"><i class="fab fa-twitter"></i>Twitter</a></li>
    <?php endif;?>

    <?php if($options->get( 'linkedin_sharing' )):?>
    <li class="linkedin"><a href="http://www.linkedin.com/shareArticle?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="fab fa-linkedin"></i>Linkedin</a></li>
    <?php endif;?>

    <?php if($options->get( 'pinterest_sharing' )):?>
    <li class="pinteret"><a href="https://pinterest.com/pin/create/bookmarklet/?url=<?php echo esc_url(get_permalink($PostID)); ?>&description=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="fab fa-pinterest"></i>Pinterest</a></li>
    <?php endif;?>

    <?php if($options->get( 'reddit_sharing' )):?>
    <li class="android"><a href="http://reddit.com/submit?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="fab fa-reddit"></i>Reddit</a></li>
    <?php endif;?>

    <?php if($options->get( 'tumblr_sharing' )):?>
    <li class="dribbble"><a href="http://www.tumblr.com/share/link?url=<?php echo esc_url(get_permalink($PostID)); ?>&name=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="fab fa-tumblr"></i>Tumblr</a></li>
    <?php endif;?>

    <?php if($options->get( 'digg_sharing' )):?>
    <li class="rss"><a href="http://digg.com/submit?url=<?php echo esc_url(get_permalink($PostID)); ?>&title=<?php echo esc_attr($post_slug=$PostName); ?>"><i class="fab fa-digg"></i></a>Digg</li>
    <?php endif;?>
