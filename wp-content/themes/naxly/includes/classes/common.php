...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => naxly_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => naxly_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => naxly_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => naxly_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => naxly_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => naxly_meta( 'banner_elementor_template', $page_id ),
				'banner'            => naxly_set( naxly_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => naxly_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => naxly_meta( 'banner_page_banner', $page_id ),
			];
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : naxly_the_title( $template ),
				'banner'        => naxly_set( $bg, 'url' ),
	...
	public function single( $template = 'single' ) {
		...
		$return = [
			...
			'banner_elementor'  => naxly_meta( 'banner_elementor_template', $page_id ),
			'banner'            => naxly_set( naxly_meta( 'banner_page_background', $page_id ), 'url' ),
			'title'             => ($title) ? $title : esc_html__('Standard Post', 'naxly'),
			'enable_banner'     => naxly_meta( 'banner_page_banner', $page_id ),
