 ...
 */
function naxly_unset_default_widgets_args( $widget_areas ) {
...
add_filter( 'merlin_unset_default_widgets_args', 'naxly_unset_default_widgets_args' );
 ...
 */
function naxly_child_functions_php( $output, $slug ) {
...
add_filter( 'merlin_generate_child_functions_php', 'naxly_child_functions_php', 10, 2 );
 ...
 */
function naxly_local_import_files() {
	return array(
		array(
			'import_file_name'         => esc_html__('Main Demo', 'naxly'),
			...
			'local_import_redux'       => array(
				array(
					...
					'option_name' => 'naxly_options',
				),
			),
			'local_import_file'        => trailingslashit( get_template_directory() ) . 'demo-import/content/content.xml',
			'import_preview_image_url' => get_template_directory_uri() . '/screenshot.png',
			'import_notice'            => esc_html__( 'After you import this demo, you will have to setup the slider separately.', 'naxly' ),
			'preview_url'              => 'http://commonsupport.net/newwp/naxly/',
		),
	);
}

add_filter( 'merlin_import_files', 'naxly_local_import_files' );

/**
 * Execute custom code after the whole import has finished.
 */
function naxly_after_import_setup() {
	...
	$front_page_id = get_page_by_title( 'Data Science' );
	$blog_page_id  = get_page_by_title( 'Modern With S/B' );
...
add_action( 'merlin_after_all_import', 'naxly_after_import_setup' );
 ...
 */
function naxly_mime_types($mimes) {
 $mimes['svg'] = 'image/svg+xml';
 return $mimes;
}
add_filter('upload_mimes', 'naxly_mime_types');